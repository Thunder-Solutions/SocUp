...
export const bool = (val: unknown): boolean | TypeError => {
	...
	if (typeof _val === 'string' && !strValues.includes(_val)) {
		return new TypeError(
			'Could not convert string to boolean because the string was not empty and neither "true" nor "false"',
		);
	}
...
export const getURL = (
	href: string,
	{ relativeTo = 'current', basePath: _basePathOverride = '' }: GetURLOptions = {},
): URL | null => {
	if (!href) return null;
	const absRegex = /^([^/]+?)\/\//;
