...
const Checkbox = (props: CheckboxComponentProps) => {
	...
	const { onChange = NOOP, label = '', required = false, checked: _checked = false } = props;
	...
	return (
		<Label label={label} required={required} inline={true}>
			<input
				...
				checked={checked}
			/>
			<span tabIndex={0} className={className} onKeyDown={handleKeyDown} data-is-checkable="true" onBlur={handleBlur} />
		</Label>
	);
};
