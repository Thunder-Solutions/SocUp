...
const Button = ({ children, className = '', type = 'submit', importance, ...props }: ButtonComponentProps) => {
	...
	return (
		<button {...props} type={type} className={buttonClass}>
			{children}
		</button>
	);
};
...
export type ButtonLinkProps = PropsWithChildren<
	RawButtonLinkProps & Omit<LinkComponentProps, keyof RawButtonLinkProps>
>;
...
const ButtonLink = ({ children, className = '', wrapperClass = '', type = '', ...props }: ButtonLinkProps) => {
	...
	return (
		<Link {...props} type="none" className={buttonClass} wrapperClass={wrapperClass}>
			{children}
		</Link>
	);
};
